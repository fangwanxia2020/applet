<template>
	<view class="content">
		<u-search placeholder="请输入任务名称" bg-color='#ffffff'  @custom='search' v-model='obj.query.name' @search='search' ></u-search>
		<view class="search">
			<view class="uni-px-5 uni-pb-5 search-box">
			<uni-data-select
			:value="obj.query.commit"
			:localdata="auditRange"
			placeholder='审核结果'
	:clear="false"
	@change="change1"
			></uni-data-select>
...
<view style='margin-bottom: 100rpx;'>
	<audit-list :taskObj='item' @changeEnable='changeEnable' :type="50" v-for="(item,index) in taskArry" :key='index'></audit-list>
...
<script>
	...
	import {getTask,getTaskType} from "@/api/pagesA/completed.js"
	
	export default{
		components:{inputBox,selectBox,auditList},
		data(){
			return{
		...
		methods:{
			...
			async  getTaskList(){
		const res =	await getTask(this.obj)
