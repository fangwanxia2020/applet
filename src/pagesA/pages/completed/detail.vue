			...
			<custom-tab-pane label="任务信息" name="c1_1">
				...
				<custom-tabs type="c2" @change="changeIndex" class="c2">
					<custom-tab-pane label="基础信息" name="c2_1">
						...
						<view :class="commitName =='已通过'?'tips-blue':'tips-red'" >
							<view :class="commitName =='已通过'?'tips-1-blue':'tips-1-red'">
								<view  :class="commitName =='已通过'?'tips-2-blue':'tips-2-red'">
									<text>{{commitName}}</text>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.id = option.id
			this.commitName = option.commit =='6'?'被驳回':option.commit=='7'?'已通过':''
