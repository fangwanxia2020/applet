<template>
	...
	<!-- 	<view class="img-handel">
			<view class="" v-if="info.id" style="margin-right: 24rpx;"  @click="handleConcern()">
				<image style="width: 54rpx;height: 48rpx;" v-if="info.isTaskPsnAttention == 0" :src="concern" mode="widthFix"></image>
				<image style="width: 54rpx;height: 48rpx;" v-else :src="concern_hl" mode="widthFix"></image>
			</view>
			<view class="" v-if="viewType == 1 && info.id"  @click="handleUrge()">
				<image style="width: 48rpx; height: 56rpx;"  v-if="urge == 0" :src="remind"   mode="widthFix"></image>
				<image style="width: 48rpx; height: 56rpx;" v-else :src="remind_hl"  mode="widthFix"></image>
			</view>
		</view> -->
		...
		methods: {
			...
			async handleConcern(){
				...
				if(this.sonType == 1){ //子取消或者新增关注
					this.info.isTaskPsnAttention ? data.id = this.info.taskPsnAttentionId : data.taskInstanceId = this.info.id
				}else{ //父取消或者新增关注
					this.info.isTaskPsnAttention ? data.id = this.info.taskPsnAttentionId : data.taskId = this.info.id
				}
				const res = await addAndCancel(data)
				if(res.msg == '操作成功'){
					this.info.isTaskPsnAttention = ! this.info.isTaskPsnAttention
					let info
					this.sonType == 1 ? info = await respTaskDetails(this.id) : info = await getInfoTask(this.id)
					this.info = info.data
				}
			},
			//督办
