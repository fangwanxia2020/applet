<template>
    <view class="box">
				...
				<custom-tab-pane label="任务信息" name="c1_1">
					<view class="list-content">
						...
						<custom-tabs type="c2"  @change="changeIndex">
								...
								<view class="c2_1_content">
									<view class="">
										...
										<view class="list-info-list">
											...
											<span>{{personName || infoData.personName}}</span>
										</view>
										<view class="list-info-list">
										...
										<view class="list-info-plan">
											...
											<view class="plan_strip">
												<u-line-progress activeColor="#026DFF" :percentage="progress||0" :showText="false"></u-line-progress>
							...
							<custom-tab-pane label="多人负责" name="c2_3"  v-if="tableDataList.length > 1 && viewType != 1">
								<view class="c2_1_content">
									<uni-table  ref="table" :stripe='false'  emptyText="暂无更多数据" >
										...
										<uni-tr v-for="(item, index) in tableDataList" :key="index">
											...
											<uni-td class="table-count">
												<view class="name"  style="color: #026DFF;" @click="next(`/pages/jobJacket/detail?id=${item.id}&taskId=${item.taskId}&personId=${item.personId}&sonType=1&viewType=1`)">查看</view>
											</uni-td>
										</uni-tr>
									</uni-table>
								</view>
		...
		<view class='next'  v-if="info.id">
			<view class="next-btn-left" @click="handleConcern()">
				<image style="width: 27rpx;height: 24rpx;"  :src="concern" mode="widthFix" v-if="info.isTaskPsnAttention == 0"></image>
				<image style="width: 27rpx;height: 24rpx;"   :src="concern_hl" mode="widthFix" v-else></image>
			</view>
			<view class="next-btn-right" @click="handleUrge()"  v-if="viewType == 1 && info.id">
				<image style="width: 27rpx;height: 24rpx;"  v-if="!urge" :src="oversee" mode="widthFix"></image>
				<image style="width: 27rpx;height: 24rpx;" v-else :src="oversee_hl" mode="widthFix"></image>
			</view>
		</view>
...
<script>
	import {respTaskDetails,respTaskInstance,getInfoByTaskId,taskLife,getSupportingInformation,fileInfo,urgeTask} from "@/api/pagesA/havePublished"
	import {getRespCommitRec} from "@/api/pagesA/audit.js"
	import {addAndCancel,getTaskProgress,getTaskPersonName} from "@/api/pages/jobJacket"
	...
	export default {
		...
		async onLoad(options) {
			const {id,taskId,sonType,selectPersonType,viewType,personId} = options
			this.viewType = viewType
			this.id = id
			this.taskId = taskId
			this.sonType = sonType
			this.selectPersonType = selectPersonType || ''
			let info,progress,personName
			if(sonType){
				info = await getInfoByTaskId(taskId)
				progress = await getTaskProgress(taskId,personId)
				personName = await getTaskPersonName(taskId,personId)
			}else{
				info = await getInfoByTaskId(id)
				progress = await getTaskProgress(id)
				personName = await getTaskPersonName(id)
			}
			this.personName = personName.data
			this.taskProgress = progress.data
		...
		methods: {
			async getRespCommitRec() {
				const res = await getRespCommitRec({
					taskId: this.id,
					taskInstanceId: this.info.taskInstanceId
				})
			...
			taskLife(){
				let data = {}
				this.sonType == 1? data.id = this.id : data.taskId = this.id
			...
			async handleConcern(){
				...
				if(this.sonType == 1){ //子取消或者新增关注
					this.info.isTaskPsnAttention ? data.id = this.info.taskPsnAttentionId : data.taskInstanceId = this.id
				}else{ //父取消或者新增关注
					this.info.isTaskPsnAttention ? data.id = this.info.taskPsnAttentionId : data.taskId = this.id
				}
				const res = await addAndCancel(data)
				if(res.msg == '操作成功'){
					this.info.isTaskPsnAttention = ! this.info.isTaskPsnAttention
					let info
					this.sonType == 1 ? info = await getInfoByTaskId(this.taskId) : info = await getInfoByTaskId(this.id)
					this.info = info.data
				}
			},
			//督办
			async handleUrge(){
				if(this.urge) return
				let data = {}
				this.sonType == 1? data.taskInstanceId = this.id : data.taskId = this.id
			...
			getSupportingInformation(){
				let data = { [this.sonType?'taskInstanceId':'taskId']:this.id ,selectPersonType:this.selectPersonType}
