...
<script>
	import {orgList,getListByAccess,addTask,getInfoTask} from "@/api/pagesA/draftBox"
	import {breakTask} from "@/api/pages/jobJacket"
	import crewRadio from "@/components/crew-radio/crew-radio"
	import crewCheckbox from "@/components/crew-checkbox/crew-checkbox"
	export default {
		components:{
			crewRadio,
			crewCheckbox
			...
			handleAccess(dataType){
			 ...
			 handleIssue(){
				...
				this.$refs.taskForm.validate().then(async res => {
					...
					const res1 = await breakTask(this.form)
					uni.navigateBack(1)
				}).catch(errors => {
					// uni.$u.toast('校验失败')	
				})
